...
In the last section you created a self-signed CA for the Rsyslog service, and then a certificate for the rsyslog service

In this section you will use the same CA you already created to create a certificate for your (not-yet-created) HPVS 2.1.3 GREP11 Server.
Your GREP11 Server will be a client to the rsyslog service. (It serves clients who want to make GREP11 requests, but it is a client to the rsyslog service.  It's okay, I'm confused too.)

It can be a little confusing as you'll be switching between your userid on the host and then your userid on your Ubuntu KVM guest that hosts the rsyslog service. Please read the instructions carefully.
...
You should see output similiar to what is shown above on the RHEL 8.5 host.  This same command was very quiet on your Ubuntu KVM guest.
...
TODO:  set up infrastructure to support above IP variable
...
Put on your CA hat and log in to your Ubuntu KVM guest 

   ``` bash
   ssh -p ${Student_SSH_Port} -l student 192.168.22.64
   ```

You are now the CA _registrar_ (certification authority-speak for _administrator_). Switch to your working directory and find the certificate signing request(CSR) that your customer- which, in this lab, is you- sent to you.  
...
???- example "Example human-readable display of CSR"
      ...
      ```
You've done your due diligence-  you've done a background check on the customer, checked their reviews on Yelp and NextDoor, looked at their Facebook page and LinkedIn profiles.  You're a little concerned with some of those college fraternity party pictures on Facebook, but, what the heck, their check has cleared the bank, so you decide to go ahead and _mint_ the certificate (_mint_ being certification authority-speak for _create_):
...
???- example "Example output from displaying the certificate"
...
TODO: set up infrastructure to support env variable used below
...
Hang up your CA registrar hat and log out of your Ubuntu KVM guest:
...
???- example "Example display of certificate"
      ...
      subject=C = US, O = IBM WSC zSystems and LinuxONE, CN = SE-enabled HPVS 2.1.3 Grep11 Server
      ```
Click the _Next_ link at the bottom of the page to continue to the next part of the lab, where you will create the _contract_ that HPVS 2.1.3 expects, so that you can run your GREP11 Server.
