...
1. application owner can encrypt their piece of the contract such that it can only be decrypted within the HPVS 2.1.4 runtime
...
4. the _systems administrator_ combines the two sections and signs the resultant contract so that it can be verified by the HPVS 2.1.4 runtime.
...
It will be called later by another script.  Comments have been added to help explain what the script does. 
	...
	# provided with the Hyper Protect Virtual Servers v2.1.4 product
	#
...
## Create the startup file for the HPVS 2.1.4 GREP11 guest

1. Create a copy of the `user_data.yaml` file that your created

	The contract that you just created is going to be packaged with some other files into a startup file for the HPVS 2.1.4 guest that will run your GREP11 Server. One of the files expected is a file named `user-data` that is just a copy of the `user_data.yaml` file that was just created
